...
**Exercise 2**: What vertical temperature difference is necessary across the
microlayer (bottom 1 mm of the atmosphere) to conduct 300 $W m^{−2}$ of heat
flux?

**Exercise 3**: Find the effective surface heat flux over a forest at neutral
...
**Exercise 4**: Given the following measurements from an instrumented tower,
...
**Exercise 5**: If the sensible heat flux is 300 $W m^{-2}$ and the latent heat
flux is 100 $W m^{-2}$, what is the Bowen-ratio? What is the likely surface
type?

**Exercise 6**: Find the drag coefficient (bulk coefficient) in statically neutral conditions to
...
**Exercise 7**: On an overcast day, a wind speed of 6 $m/s$ is measured with an
anemometer located 10 $m$ over ground within a corn field. What is the wind
speed at 25 $m$ height?

**Exercise 8**: Wind speed is measured at two different heights. Find the
...
**Exercise 9**: Given $K_H=5~m^2 s^{-1}$ for turbulence within a stable
